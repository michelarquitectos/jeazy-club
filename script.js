...
const unlockMemberMenu=()=>{const menuStep=document.querySelector('#menuStep');menuStep.classList.remove('locked');menuStep.classList.add('completed');menuStep.setAttribute('aria-disabled','false');document.querySelector('#menuState').textContent='Disponible';document.querySelector('#lockedMessage').hidden=true;document.querySelector('#privateMenu').hidden=false;document.querySelector('#documentStep').classList.add('completed');document.querySelector('#documentState').textContent='Completado'};
document.querySelector('#leadForm').addEventListener('submit',event=>{event.preventDefault();localStorage.setItem('jeazy-registered','yes');event.currentTarget.querySelector('.form-status').textContent='Registro completado. Continúa con el documento de membresía.';setTimeout(()=>{window.location.hash='area-socios'},450)});
document.querySelector('#documentForm').addEventListener('submit',event=>{event.preventDefault();localStorage.setItem('jeazy-document-signed','yes');document.querySelector('#documentStatus').textContent='Documento de demostración completado. El menú ya está disponible.';unlockMemberMenu()});
if(localStorage.getItem('jeazy-document-signed')==='yes')unlockMemberMenu();
